        ...
        for idx, nombre_zip in enumerate(zips_tipo2):
            try:
                ...
                with zipfile.ZipFile(datos, 'r') as zf_inner:
                    ...
                    if not debug_hecho:
                        log(f'=== PRIMER ZIP INTERNO ===')
                        log(f'Nombre: {nombre_zip}')
                        log(f'Archivos ({len(archivos)}): {archivos}')
                        # Mostrar primeras 2 lineas de CADA archivo
                        for arch in archivos:
                            try:
                                with zf_inner.open(arch) as f:
                                    primeras = f.read(300).decode('utf-8', errors='replace')
                                lineas = primeras.replace('\r\n','\n').split('\n')
                                log(f'  [{arch}] header: {lineas[0][:150]}')
                                if len(lineas) > 1:
                                    log(f'  [{arch}] fila1:  {lineas[1][:150]}')
                            except Exception as e:
                                log(f'  [{arch}] ERROR: {e}')
