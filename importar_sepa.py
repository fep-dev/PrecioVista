...
def procesar_csv(zip_path, usuario_id):
    ...
    with zipfile.ZipFile(zip_path, 'r') as zf:
        # El ZIP puede contener CSV o archivos sin extensión
        # Buscar primero .csv, luego cualquier archivo que no sea directorio
        nombres = zf.namelist()
        log(f'Archivos en ZIP: {nombres}')
        
        csv_files = [f for f in nombres if f.lower().endswith('.csv')]
        if not csv_files:
            # Intentar con cualquier archivo que no sea directorio
            csv_files = [f for f in nombres if not f.endswith('/') and '.' not in f.split('/')[-1]]
        if not csv_files:
            # Tomar el archivo más grande como fallback
